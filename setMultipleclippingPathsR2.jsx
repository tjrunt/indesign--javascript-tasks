...
function main(){
    ...
    if(applyPathTO == 'All images'){
        mySetPaths =  setPaths(images, all, choosenPath)  
        }
    
     else if(applyPathTO == 'Selected images'){
         mySetPaths = setPaths(images, inx, choosenPath) 
     }
         ...
         while(inx.length > 0){
            ...
            mySetPaths = setPaths(images, myImageIndex, choosenPath)
          }   
    
            errorDialog(mySetPaths)
  }
...
function setPaths(images,  myImageIndexList, choosenPath){
    //builds a list of images without paths that match  out standart naming convention
    var  missingPaths = new Array;
    if(choosenPath == 'NONE'){
        ...
        for (var count =0; count < images.length; count++){ 
             ...
             images[myImageIndexList[count]].clippingPath.appliedPathName = 'ol';
                     ...
                     imageName =  images[myImageIndexList[count]].itemLink.name
                     missingPaths.push(imageName);
       ...
       for (var count =0; count < images.length; count++){ 
            ...
            images[myImageIndexList[count]].fit(FitOptions.FRAME_TO_CONTENT);
                    ...
                    imageName = myImageIndexList[count].itemLink.name
                    missingPaths.push(imageName);
...
function errorDialog(missingPaths){
     var listImages = new Array
     msg = 'DONE \n The following links did not \n  contain paths named \'ol\' or \'ol+shd\' : \n';
    if (missingPaths.length > 0){
    for (var i = 0; i <  missingPaths.length; i++){
        temp = '"' + missingPaths[i]+'"'
        listImages.push(temp);
        listImages.push(' \n');
        }
    x = msg + listImages
    alert (x, x)
    }else{
     x =  'DONE \n your image paths are now set'
     alert (x, x)  
        }    
    }
function getPsdLayers(myImage){
    //var thePaths = theImage.clippingPath.photoshopPathNames
    
    theLayers = myImage.graphicLayerOptions.graphicLayers
